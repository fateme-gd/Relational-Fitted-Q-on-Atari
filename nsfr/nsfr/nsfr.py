...
class NSFReasoner(nn.Module):
    ...
    def compute_init_v(self, x):  
        #Fateme: just compute the initial valuation
        zs = x
        # convert to the valuation tensor
        self.V_0 = self.fc(zs, self.atoms, self.bk)
    ...
    def print_valuations_input(self, V, predicate: str = None, min_value: float = 0, print_state = None):
        # print('===== VALUATIONS =====')
        valuation = V
        symbolic_states = []
            ...
            for i in idxs:
                ...
                if value >= min_value:
                    ...
                    if predicate is None or predicate == atom.pred.name:
                        ...
                        if print_state:
                            print(f"{value:.3f} {atom}")
        return symbolic_states, goal_reached
