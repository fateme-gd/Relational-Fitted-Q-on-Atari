...
class GBQL(Trainer):
    ...
    def fit_q(self, train, path=None, save=False):
        ...
        # load cache
        states = self.env.get_state_space()                     #env - in block env this method is not implemented and returns none
        if states is not None:
            test = Database()     # a class of pos, neg and facts 
            for i, state in enumerate(states):
                print("observe is:\n", self.env.observe(state, state_id=F"s{i}") )
                test.facts += self.env.observe(state, state_id=F"s{i}")       #env
                test.pos += self.env.all_actions(state, state_id=F"s{i}", regression=True)    #env
            if self.additional_facts is not None:
    ...
    def generate_batch(self, train_batch, batch_size=10, q_function=None):
        ...
        for i in range(batch_size):
            ...
            self.env.reset()     #in getout/nudge it resets the setting and also return logic and neural state g
            current_state = self.env.state      #env
            ...
            while not done:
                ...
                next_state = self.env.apply_action(action)   #env
                reward = self.env.get_reward(current_state, action, next_state)    #env
                if self.env.is_goal_state(next_state):                               #env
    ...
    def get_qvalue(self, state, action, q_function=None, env=None):
        ...
        if q_function is None:
            return 0.0
        possible_actions = self.env.all_actions(state)            #env
    ...
    def train(self):
        ...
        for i in gt.timed_for(range(self.n_iterations), save_itrs=True):
            ...
            if i % self.test_gap == 0:
                ...
                paths = self.evaluate(self.n_eval_traj, updated_q) #q_function gets updated here
    ...
    def _log_stat(self, q_function, training_stats, paths, itr):
        ...
        evaluation_stats = self.env.get_diagnostics(paths)     #env
    ...
    def add_sample(self, train_batch, state, action, q_value, state_id='0'):
        train_batch.facts += self.env.observe(state, state_id=f"s{state_id}")         #env
