...
class BoostedRDNRegressor(BaseBoostedRelationalModel):
    ...
    def fit(self, database, path=None, preserve_data=False):
        ...
        print(self.file_system.files.TREES_DIR.value)
        # Read the trees from files.
        _estimators = []
        ...
        for _tree_number in range(self.n_estimators):
            for tar in self.target.split(","):
                with open(
                    self.file_system.files.TREES_DIR.value.joinpath(
                        "{0}Tree{1}.tree".format(tar, _tree_number)
                    )
                ) as _fh:
                    _estimators.append(_fh.read())
    ...
    def predict(self, database):
        ...
        # Collect the classifications.
        for tar in self.target.split(","):
            _results_db.append(self.file_system.files.TEST_DIR.value.joinpath(
                "results_" + tar + ".db"
            ))
        
        for db in _results_db:
