...
from environments_old.environment import Environment, State, Task
import numpy as np
import re
...
class BlocksWorldStack(Environment, Task):    
    ...
    def all_actions(self, state=None, state_id="_", regression=False):     #all possible ground actions in an state
        if state is None:
            state = self._state
        possible_actions = []
        # predicate = "{MOVE}({state_id},{i},{j})"
        predicate = "{MOVE}({state_id})"
        if regression:
            predicate = "regressionExample({MOVE}({state_id},{i},{j}),0.0)."
            # predicate = "regressionExample({MOVE}({state_id}),0.0)."
        for from_tower in state.towers:
            if len(from_tower) > 1 and len(state.towers) > 1:
                ...
                # possible_actions.append(predicate.format(MOVE=MOVE,state_id=state_id))
            for i in from_tower:
