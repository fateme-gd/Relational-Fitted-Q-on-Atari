...
import torch as th
from OC_Atari.ocatari.ram.seaquest import MAX_NB_OBJECTS
import gymnasium as gym
from HackAtari.hackatari.core import HackAtari
...
class VectorizedNudgeEnv(VectorizedNudgeBaseEnv):
    ...
    def __init__(
        ...
        self.envs = [
            HackAtari(
                ...
                rewardfunc_path="ins/envs/seaquest/blenderl_reward.py",
    ...
    def step(self, actions, is_mapped: bool = False):
        if isinstance(actions, int):
            assert self.n_envs == 1, "Invalid number of actions: n_actions is 1 but n_envs is {}".format(self.n_envs)
            actions = [actions]
        else:
            assert len(list(actions)) == self.n_envs, "Invalid number of actions: n_actions is {} and n_envs is {}".format(len(actions), self.n_envs)
