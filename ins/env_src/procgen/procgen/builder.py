...
def _attempt_configure(build_type, package):
    if "PROCGEN_CMAKE_PREFIX_PATH" in os.environ:
        ...
        if conda_exe is not None:
            ...
            # )
            conda_info = {"active_prefix": "'C:\\Users\\tongz\\anaconda3\\envs\\procgen'",
                          "conda_prefix": None}
...
def build(package=False, debug=True):
    ...
    with chdir(build_dir), global_build_lock:   #chdir changes the os.dir to the new_dir, but in the scope of with! 
