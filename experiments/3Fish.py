...
for i in range(n_iter):
    ...
    train_env = NudgeBaseEnv.from_name('threefish', mode=algorithm, **variant["env_kwargs"])

    agent = LogicPPO(train_env, rules = "threefish_bs_rf1", lr_actor = 0.001, lr_critic = 0.0003, optimizer = Adam,
                         gamma = 0.99, epochs = 20, eps_clip = 0.2, device = "cpu")

    modes= ["type(-object, +type).",
            "closeby(+object, +object).",
            "on_top(+object, +object).",
            "at_bottom(+object, +object).",
            "bigger_than(+object, +object).",
            "on_left(+object, +object).",
            "on_right(+object, +object).",
            "high_level(+object, +object).",
            "low_level(+object, +object)."
            ]

    bk = Background(modes, **variant['bk_kwargs'])

    test_env = NudgeBaseEnv.from_name('threefish', mode=algorithm, **variant["env_kwargs"])
    ###############
